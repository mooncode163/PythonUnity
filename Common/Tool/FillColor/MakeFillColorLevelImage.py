...
class MakeGameLevelImage():  
    ...
    def ScanFiles(self,dir):
        for file in os.listdir(dir):
            ...
            if os.path.isfile(path): 
                ext = self.GetFileExt(path)
                if ext=="png" or ext =="jpg" or ext =="jpeg":
                    print("ScanFiles=",file)
                    # self.FormatImage(path,path)
                    ...
                    if isfilter==False:
                        ...
                        self.MakeThumb(path,w,h,type)

            #目录嵌套
            if os.path.isdir(path): 
                self.ScanFiles(path)

    def ScanPlaces(self,dir):
        for file in os.listdir(dir):
            # 过滤文件
            if file == "Thumbs.db":
                continue
            path = os.path.join(dir,  file)  
            if os.path.isdir(path): 
                self.fileJosn = "item_"+self.GetFileNamePath(file)+".json"
                if os.path.exists(self.fileJosn)==False:
                    self.pathPlace = path
                    self.listFile = []
                    self.ScanFiles(path+"/draw")
    ...
    def MakeThumb(self,filein, width, height, type):
        w = width
        h = height
        tosize = 256
        if w>h:
            ...
            h_new =tosize
            w_new = (int)(w*h_new/h)
        dirthumb = self.pathPlace+"/thumb"
        if os.path.exists(dirthumb)==False:
            os.mkdir(dirthumb)
        filein = os.path.normpath(filein)
        # print("filein=",filein)
        fileout = filein.replace("\\draw\\","\\thumb\\")
        fileout = os.path.normpath(fileout)
        # print("fileout=",fileout)
        self.ResizeImage(filein,fileout,w_new,h_new,type)

    # 非透明统一成黑色
    def FormatImage(self,filein, fileout):
        img = Image.open(filein)
        img = img.convert("RGBA")
        datas = img.getdata()
        newData = list()
    ...
    def Run(self): 
        ...
        self.ScanPlaces(self.GetCurPath())
